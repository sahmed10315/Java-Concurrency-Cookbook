package threads.examples.conditions;
...
public class SingleCondition {
	// first create a thread that waits for Joe to arrive and then create
	// new Train threads
	public static void main(String[] args) throws InterruptedException {
		// we are waiting before the trains start coming
		new RailwayStation.WaitForJoe().start();
		// Trains are separate threads - they can arrive in any order
		new RailwayStation.Train("IC1234 - Paris to Munich").start();
		new RailwayStation.Train("IC2211 - Paris to Madrid").start();
		new RailwayStation.Train("IC1122 - Madrid to Paris").start();
		new RailwayStation.Train("IC4321 - Munich to Paris").start();
	}
}
